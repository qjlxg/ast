...
SIGNAL_CN_MAP = {
    ...
    'Volume_Surge': '成交量突增',              # **新增**
    'Short_Term_Bull_Arrangement': '短期多头排列' # **新增**
}
# --------------------
...
def fetch_stock_ohlc(stock_code, stock_name, end_date):
    ...
    try:
        ...
        required_cols = ['Open', 'Close', 'High', 'Low', 'Volume'] # **新增 Volume 检查**
...
def calculate_all_indicators(df):
    """Calculates MA, MACD, KDJ, RSI, and Volume MA indicators."""
    ...
    # 5. Volume Moving Average (新增)
    df['MA_Volume_5'] = df['Volume'].rolling(window=5, min_periods=1).mean() # **新增**
    
    return df

# --- Screening Logic Function (增加两个信号) ---
def get_uptrend_signals(df):
    ...
    # D. Volume Signal (新增高成功率信号)
    # Volume Surge: 当日成交量高于5日均量1.5倍（参数可调）
    volume_magnification_ratio = 1.5 
    if latest['Volume'] > (latest['MA_Volume_5'] * volume_magnification_ratio):
        signals['Volume_Surge'] = True # **新增信号**
        
    # E. Trend Strength Signal (新增高成功率信号 - 均线多头排列)
    # MA5 > MA20 > MA60 (多头排列确认，中短期趋势强势)
    if latest['MA5'] > latest['MA20'] and latest['MA20'] > latest['MA60']:
        signals['Short_Term_Bull_Arrangement'] = True # **新增信号**
