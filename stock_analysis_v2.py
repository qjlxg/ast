...
# 原始数据文件必须包含的列名 (新增 '成交量' 以判断停牌)
REQUIRED_RAW_COLUMNS = ['日期', '收盘', '最高', '最低', '股票代码', '成交量']
...
def add_technical_indicators(df):
    ...
    # 确保用于计算的列是数字，且不是 NaN
    required_calc_cols = ['收盘', '最高', '最低']
    df = df.dropna(subset=required_calc_cols) 
...
def main():
    ...
    for code_str, stock_name in stock_map.items():
        ...
        try:
            ...
            prev_raw = df_raw.iloc[-2]
            
            latest_close = latest_raw['收盘']
            prev_close = prev_raw['收盘']
            latest_volume = latest_raw['成交量']
            
            # 过滤 C1: 停牌判断（通常以成交量为 0 或极小值判断）
            if pd.isna(latest_volume) or latest_volume <= 0:
                # print(f"[DEBUG] ⚠️ {code_str} ({stock_name}): 最新成交量为 {latest_volume}，可能停牌，跳过。")
                continue
            
            # 过滤 C2: 涨停判断（通常无法在涨停板买入，保守使用 9.5%）
            if not pd.isna(latest_close) and not pd.isna(prev_close) and prev_close > 0:
                daily_change = (latest_close / prev_close) - 1
                
                # 过滤涨停 (A股普遍是 +/-10%，使用 9.5% 作为保守过滤阈值)
                if daily_change >= 0.095: 
                    # print(f"[DEBUG] ⚠️ {code_str} ({stock_name}): 今日涨停 ({daily_change:.2%})，跳过。")
                    continue
