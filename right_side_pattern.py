...
def process_stock_file(stock_code):
    ...
    if not os.path.exists(filepath):
        # print(f"警告: 股票数据文件 {filepath} 不存在，跳过。")
        return None
...
def find_and_read_signals():
    ...
    for date in search_dates:
        ...
        year_month_dir = date.strftime("%Y%m")
        
        # 构造预期的文件路径
        # 格式为: buy_signals/YYYYMM/YYYYMMDD.csv
        signal_dir = os.path.join(BUY_SIGNALS_BASE_DIR, year_month_dir)
        ...
        if os.path.exists(signal_filepath):
            ...
            try:
                ...
                if '股票代码' in signal_df.columns:
                    stock_codes = signal_df['股票代码'].astype(str).tolist()
                    # 确保代码是唯一的
                    return list(set(stock_codes))
                else:
                    print(f"错误: 信号文件 {signal_filepath} 中未找到 '股票代码' 列。")
                    return []
            except Exception as e:
                print(f"读取信号文件 {signal_filepath} 时出错: {e}")
                return []

    print("警告: 在最近三天内（今天、昨天、前天）未找到符合格式的信号文件。")
    return []

# --- 主执行逻辑 (多进程运行) ---
def main_optimized():
    # 1. 查找并读取信号文件中的股票代码
    stock_codes_to_process = find_and_read_signals()
